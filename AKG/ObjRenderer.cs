...
public unsafe class ObjRenderer
{
    ...
    {
        if (Model.Vertices.Count == 0 || !_needsModelAdjustment)
            return;
        ...
        foreach (Vector4 vertex in Model.Vertices)
        {
        ...
        _lastVertexCount = Model.Vertices.Count;
        _matricesDirty = true;
    ...
    private void RenderOptimized()
    {
        try
        {
            if (Model.Vertices.Count == 0 || Model.Faces.Count == 0)
                return;

            
            if (_lastVertexCount != Model.Vertices.Count)
            {
            ...
            try
            {
                ...
                _screenPoints.Clear();
                if (_screenPoints.Capacity < Model.Vertices.Count)
                {
                    _screenPoints.Capacity = Model.Vertices.Count * 2;
                }
    ...
    private void DrawFaceOptimized(FaceIndices[] face)
    {
        ...
        for (int i = 0; i < face.Length; i++)
        {
            ...
            int idx1 = face[i].VertexIndex;
            int idx2 = face[j].VertexIndex;
        
            if (idx1 >= 0 && idx1 < _screenPoints.Count && 
                idx2 >= 0 && idx2 < _screenPoints.Count)
            {
                Point p1 = _screenPoints[idx1];
                Point p2 = _screenPoints[idx2];
